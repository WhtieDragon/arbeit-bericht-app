...
interface ReportEditDialogProps {
  ...
  onSave: (updatedReport: WorkReport) => void;
}

const ReportEditDialog = ({ report, open, onOpenChange, onSave }: ReportEditDialogProps) => {
  ...
  const [selectedColleagues, setSelectedColleagues] = useState<Colleague[]>(report.colleagues || []);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Arbeitsbericht bearbeiten</DialogTitle>
          <DialogDescription>
            Bearbeite die Details des Arbeitsberichts vom {new Date(report.date).toLocaleDateString()}.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {colleagues.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">Kollegen für diesen Tag</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
